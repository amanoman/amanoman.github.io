...
def entry_signal( data,last_data,flag ):
	...
	if signal["side"] == "BUY":
		print_log("過去{0}足の最高値{1}円を、直近の価格が{2}円でブレイクしました".format(buy_term,signal["price"],data[judge_price["BUY"]]))
		# フィルター条件を確認
		...
		if lot >= 0.01:
			print_log("{0}円あたりに{1}BTCで買いの成行注文を出します".format(data["close_price"],lot))
			
			# ここに買い注文のコードを入れる(バックテストのためコメントアウト)
	...
	if signal["side"] == "SELL":
		print_log("過去{0}足の最安値{1}円を、直近の価格が{2}円でブレイクしました".format(sell_term,signal["price"],data[judge_price["SELL"]]))
		# フィルター条件を確認
		if filter( signal ) == False:
			print_log("フィルターのエントリー条件を満たさなかったため、エントリーしません")
			return flag
		...
		lot,stop,flag = calculate_lot( last_data,data,flag )
		if lot >= 0.01:
			print_log("{0}円あたりに{1}BTCの売りの成行注文を出します".format(data["close_price"],lot))
			
			# ここに売り注文のコードを入れる(バックテストのためコメントアウト)
...
def stop_position( data,flag ):
	...
	if flag["position"]["side"] == "BUY":
		...
		if data["low_price"] < stop_price:
			...
			print_log(str(data["low_price"]) + "円あたりで成行注文を出してポジションを決済します")
			
			# 決済の成行注文コードを入れる(バックテストのためコメントアウト)
	...
	if flag["position"]["side"] == "SELL":
		...
		if data["high_price"] > stop_price:
			print_log("{0}円の損切ラインに引っかかりました。".format( stop_price ))
			print_log(str(data["high_price"]) + "円あたりで成行注文を出してポジションを決済します")
...
def close_position( data,last_data,flag ):
	...
	if flag["position"]["side"] == "BUY":
		if signal["side"] == "SELL":
			...
			lot,stop,flag = calculate_lot( last_data,data,flag )
			if lot >= 0.01:
				print_log("さらに{0}円あたりに{1}BTCの売りの成行注文を入れてドテン出します".format(data["close_price"],lot))
	...
	if flag["position"]["side"] == "SELL":
		if signal["side"] == "BUY":
			...
			lot,stop,flag = calculate_lot( last_data,data,flag )
			if lot >= 0.01:
				print_log("さらに{0}円あたりで{1}BTCの買いの成行注文を入れてドテンします".format(data["close_price"],lot))
...
def bitflyer_market(side,lot):
	#while True:
		#try:
			...
			flag["position"]["exist"]=True
			flag["position"]["side"]=side
			flag["position"]["price"]=round(data["close_price"]+lot)
			#order_id = order["id"]
			#time.sleep(30)
...
while i < len(price):	
	...
	if data["close_time"] > last_data[-1]["close_time"]:
		last_data.append( data["close_time"] )
