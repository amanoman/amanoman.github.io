...
def log_price(data, flag):
	log = "時間： " + datetime.fromtimestamp(data["close_time"]).strftime('%Y/%m/%d %H:%M') + " 高値： " + str(
...
def entry_signal(data, last_data, flag):
	...
	if signal["side"] == "BUY":
		...
		if lot > 0.01:
			...
			flag["records"]["log"].append(
				"注文可能枚数{}が、最低注文単位に満たなかったので注文を見送ります\n".format(lot))

	if signal["side"] == "SELL":
		flag["records"]["log"].append("過去{0}足の最安値{1}円を、直近の価格が{2}円でブレイクしました\n".format(
			sell_term, signal["price"], data[judge_price["SELL"]]))

		lot, stop = calculate_lot(last_data, data, flag)
		if lot > 0.01:
			flag["records"]["log"].append(
				"{0}円で{1}BTCの売り注文を出します\n".format(data["close_price"], lot))
...
def close_position(data, last_data, flag):
	...
	if flag["position"]["side"] == "BUY":
		if signal["side"] == "SELL":
			flag["records"]["log"].append("過去{0}足の最安値{1}円を、直近の価格が{2}円でブレイクしました\n".format(
				sell_term, signal["price"], data[judge_price["SELL"]]))
			flag["records"]["log"].append(
				str(data["close_price"]) + "円あたりで成行注文を出してポジションを決済します\n")
			...
			if lot > 0.01:
				flag["records"]["log"].append(
					"さらに{0}円で{1}BTCの売りの注文を入れてドテンします\n".format(data["close_price"], lot))
	...
	if flag["position"]["side"] == "SELL":
		if signal["side"] == "BUY":
			...
			if lot > 0.01:
				flag["records"]["log"].append(
					"さらに{0}円で{1}BTCの買いの注文を入れてドテンします\n".format(data["close_price"], lot))
...
def stop_position(data, flag):
	...
	if flag["position"]["side"] == "BUY":
		...
		if data["low_price"] < stop_price:
			...
			stop_price = round(
				stop_price - 2 * calculate_volatility(last_data) / (chart_sec / 60))
			flag["records"]["log"].append(
				str(stop_price) + "円あたりで成行注文を出してポジションを決済します\n")
...
def backtest(flag):
	...
	records["DrawdownRate"] = round(
		records.Drawdown / records.Funds.cummax() * 100, 1)
	...
	# 月別のデータを集計する
	records["月別集計"] = pd.to_datetime(
		records.Date.apply(lambda x: x.strftime('%Y/%m')))
	...
	print("勝率               :  {}％".format(
		round(len(buy_records[buy_records.Profit > 0]) / len(buy_records) * 100, 1)))
	...
	print("勝率               :  {}％".format(
		round(len(records[records.Profit > 0]) / len(records) * 100, 1)))
	...
	print("利益合計           :  {}円".format(
		records[records.Profit > 0].Profit.sum()))
	print("損失合計           :  {}円".format(
		records[records.Profit < 0].Profit.sum()))
	...
	print("運用成績           :  {}％".format(
		round(records.Funds.iloc[-1] / start_funds * 100), 2))
