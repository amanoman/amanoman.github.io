...
def fetch_executions(timestamp, limit=1000, product_id=5): #関数の定義（仮引数はtimestamp,limit,product_idで、それぞれデフォルト値が1000と5
    retry_count = 0     #リトライカウント変数定義
    while True:         #whileループ開始
        try: #例外処理
            ...
            return [{"id": item["id"],
                     ...
                     "size": float(item["quantity"])} for item in results] #resultsの中へAPIから取得した情報を入れて、for文でまわして[id][timestamp][side][price][size]をreturnしている。リスト内包表記？forもitemを使っている。
...
if __name__ == "__main__": #インポートされても勝手に実行しないようにするおまじない
    api = Quoinex("", "") #apiからexcutionsをgetするための変数準備
    jst = timezone(timedelta(hours=9), "JST") #タイムゾーンを指定する変数準備
    datetime_format = "%Y-%m-%d %H:%M:%S" #datetimeのformatを準備
    header = ["timestamp", "datetime_jst", "open", "high", "low", "close", "volume", "buy_volume", "sell_volume", #ファイル出力で使うヘッダーをリストで準備？ここにデータをいれてリスト化？
